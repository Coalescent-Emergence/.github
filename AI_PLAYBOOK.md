...
- Social Post Writer: Generate LinkedIn/Facebook post sets in the founder's voice for Kerrigan GTM content. Standalone agent; loads Kerrigan product context docs at runtime. See `agents/social-post-writer/`.

Agent location rule
- ALL agents (prompt.md, schema.json, README.md) live in `org-dot-github/agents/<agent-name>/` — the canonical `.github` repository.
